...
<h1 align="center">2025 西安交通大学数据竞赛 · TOP 6 · 数学题图像理解</h1>
<p align="center">
  <em>基于 InternVL3.5 与 OpenMath 的"分而治之"完整方案</em>
</p>
...
传统方案用单个 VL 模型端到端处理所有题目（baseline 得分 60.5~65.1），但受限于 V100 显存，模型参数量有上限。本方案的思路是：

1. **先用 VL 模型做 OCR**：提取题目文本，同时判断是否含图表
...
在竞赛官方闭合数据集上，本方案（1.54B + 2.35B 双小模型）以 **67.4%** 的准确率取得 **TOP 6**，相比 Qwen2.5-VL-3B baseline 提升 **23.7%**。数据清洗（Input Image Cleaned）带来了额外 5.8% 的提升（61.6% → 67.4%）。
...
在自建测试集上，本方案达到 **79.5%** 准确率，**超越 Qwen2.5-VL-32B（72.5%）**——一个参数量是本方案 8 倍的模型。相比 3B baseline（45.4%），提升幅度达到 **34.1%**。这证明了「分而治之」策略的巨大优势：用对的模型做对的事，比盲目堆参数量更有效。
